...
class _TouchpadScreenState extends State<TouchpadScreen> {
  ...
  void _startInertia(double velocity) {
    ...
    _currentInertiaVelocity = velocity;
    
    _inertiaTimer = Timer.periodic(const Duration(milliseconds: 16), (timer) {
      // Firm stop threshold (80)
      if (_currentInertiaVelocity.abs() < 80) {
        timer.cancel();
        return;
      }
      
      // Slower scaling for inertia (divisor 2500)
      _sendScroll(_currentInertiaVelocity / 2500);
      
      // Friction (0.94) - "Sedikit licin" feel, more grounded
      _currentInertiaVelocity *= 0.94;
    });
  }

  void _stopInertia() {
    _inertiaTimer?.cancel();
    _currentInertiaVelocity = 0;
  }
